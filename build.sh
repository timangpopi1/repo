...
function build_clang() {
    git clone --quiet --depth=1 https://github.com/crdroidandroid/android_prebuilts_clang_host_linux-x86_clang-7284624 -b 11.0 clang-12
    ...
    export PATH="$(pwd)/clang-12/bin:$(pwd)/gcc/bin:$(pwd)/gcc32/bin:$PATH"
    export LD_LIBRARY_PATH="$(pwd)/clang-12/lib:$LD_LIBRARY_PATH"
...
New updates for #${codename} based on Linux <b>$(cat $(pwd)/out/.config | grep Linux/arm64 | cut -d " " -f3)</b> at commit $(git log --pretty=format:"%h (\"%s\")" -1)" -F chat_id=${channel_id}
