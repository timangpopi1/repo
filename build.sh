...
trigger_sha="$(echo $(git rev-parse HEAD) | cut -c 1-8)" && commit_msg="$(git log --pretty=format:'%s' -1)"
...
New build for #${codename} <b>+</b> $(cat $(pwd)/out/.config | grep Linux/arm64 | cut -d " " -f3) success at commit <a href='${my_project}/${target_repo}/commit/${trigger_sha}'>${trigger_sha}</a> (\"${commit_msg}\") | <b>SHA1:</b> <code>$(sha1sum $(echo $(pwd)/anykernel-3/*.zip ) | awk '{ print $1 }')</code>." -F chat_id=${channel_id}
