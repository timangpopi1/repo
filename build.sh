...
kmake_flags+=(AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip LD=ld.lld)
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out CC=clang "kmake_flags" CROSS_COMPILE=aarch64-linux-gnu- $kernel_defconfig
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out CC=clang "kmake_flags" CROSS_COMPILE=aarch64-linux-gnu- 2>&1| tee build.log
