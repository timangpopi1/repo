...
git clone --quiet -j64 --depth=1 --single-branch https://github.com/greenforce-project/clang-llvm clang
...
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out AR=llvm-ar CC=clang CROSS_COMPILE=aarch64-linux-gnu- NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump LD=ld.lld STRIP=llvm-strip $kernel_defconfig
...
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out AR=llvm-ar CC=clang CROSS_COMPILE=aarch64-linux-gnu- NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump LD=ld.lld STRIP=llvm-strip $kernel_defconfig && make savedefconfig
