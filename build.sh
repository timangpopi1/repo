...
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out CC=clang CROSS_COMPILE=aarch64-linux-gnu- CONFIG_TOOLS_SUPPORT_RELR=y CONFIG_RELR=y LD=ld.lld $kernel_defconfig
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out CC=clang CROSS_COMPILE=aarch64-linux-gnu- CONFIG_TOOLS_SUPPORT_RELR=y CONFIG_RELR=y LD=ld.lld 2>&1| tee build.log
