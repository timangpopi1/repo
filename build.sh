...
git clone --quiet -j64 --depth=1 --single-branch https://gitlab.com/Panchajanya1999/azure-clang clang
...
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out AR=llvm-ar CC=clang CROSS_COMPILE=aarch64-linux-gnu- CONFIG_TOOLS_SUPPORT_RELR=y CONFIG_RELR=y NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump LD=ld.lld STRIP=llvm-strip $kernel_defconfig
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out AR=llvm-ar CC=clang CROSS_COMPILE=aarch64-linux-gnu- CONFIG_TOOLS_SUPPORT_RELR=y CONFIG_RELR=y NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump LD=ld.lld STRIP=llvm-strip 2>&1| tee build.log
...
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out AR=llvm-ar CC=clang CROSS_COMPILE=aarch64-linux-gnu- CONFIG_TOOLS_SUPPORT_RELR=y CONFIG_RELR=y NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump LD=ld.lld STRIP=llvm-strip $kernel_defconfig && make savedefconfig
