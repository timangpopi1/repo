...
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out $kernel_defconfig
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out CC=clang CROSS_COMPILE=aarch64-linux-gnu- 2>&1| tee build.log
...
make -j$(nproc --all) -l$(nproc --all) ARCH=arm64 O=out CC=clang CROSS_COMPILE=aarch64-linux-gnu- $kernel_defconfig && make savedefconfig
